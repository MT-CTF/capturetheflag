...
core.register_privilege("canafk", {
	...
	on_revoke = function(name)
		if not players[name] then
			players[name] = {
				lastAction = os.clock(),
				lastPos = core.get_player_by_name(name):get_pos(),
			}
		end
	end,
})
...
core.register_on_joinplayer(function(player)
	if not core.check_player_privs(player, { canafk = true }) then
...
core.register_on_chat_message(function(playerName, message)
	...
	if not playerName or not core.get_player_by_name(playerName) or not players[playerName] then
		return
	end
