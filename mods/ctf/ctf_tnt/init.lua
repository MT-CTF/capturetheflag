...
local function destroy(drops, npos, cid, c_air, c_fire,
	...
	if node then
		...
		if minetest.registered_nodes[node.name].groups.immortal then
			return cid
		end
	end
...
local function add_effects(pos, radius, drops)
	...
	minetest.add_particlespawner({
		...
		minsize = radius * 3,
		maxsize = radius * 5,
		texture = "ctf_tnt_smoke.png",
	})

	-- we just dropped some items. Look at the items entities and pick
	-- one of them to use as texture
	local texture = "ctf_tnt_blast.png" --fallback texture
	local node
	local most = 0
...
function ctf_tnt.burn(pos, nodename)
	...
	elseif minetest.get_item_group(name, "tnt") > 0 then
		...
		minetest.sound_play("ctf_tnt_ignite", {pos = pos, gain = 1.0}, true)
		minetest.get_node_timer(pos):start(1)
	end
end

local function tnt_explode(pos, radius, ignore_protection, ignore_on_blast, owner, owner_team, explode_center)
	...
	local count = 0
	local c_tnt
	local c_tnt_burning = minetest.get_content_id("ctf_tnt:tnt_burning")
	...
	for x = -radius, radius do
		...
		if (radius * radius) / (r * r) >= (pr:next(80, 125) / 100) then
			...
			if cid ~= c_air and cid ~= c_ignore then
				local replace = destroy(drops, p, cid, c_air, c_fire,
					on_blast_queue, on_construct_queue,
					ignore_protection, ignore_on_blast, owner)
...
function ctf_tnt.boom(pos, def)
	...
	if not def.explode_center and def.ignore_protection ~= true then
		minetest.set_node(pos, {name = "ctf_tnt:boom"})
	end
	minetest.sound_play("ctf_tnt_explode", {pos = pos, gain = 2.5,
			max_hear_distance = math.min(def.radius * 20, 128)}, true)
	local drops, radius = tnt_explode(pos, def.radius, def.ignore_protection,
			def.ignore_on_blast, owner, owner_team, def.explode_center)
	-- append entity drops
...
minetest.register_node("ctf_tnt:boom", {
	drawtype = "airlike",
	inventory_image = "ctf_tnt_boom.png",
	wield_image = "ctf_tnt_boom.png",
...
minetest.register_craftitem("ctf_tnt:tnt_stick", {
	description = S("TNT Stick"),
	inventory_image = "ctf_tnt_tnt_stick.png",
	groups = {flammable = 5},
})
