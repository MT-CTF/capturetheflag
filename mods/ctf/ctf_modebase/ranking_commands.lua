...
minetest.register_chatcommand("donate", {
	description = "Donate your match score to your teammates\nCan be used only once in 10 minutes",
	params = "<player1> <score1> <player2> <score2> ... [message]",
	func = function(name, param)
		local current_mode = ctf_modebase:get_current_mode()
		...
		for i = 1, #params, 2 do
			if tonumber(params[i]) or (i + 1 <= #params and not tonumber(params[i + 1])) then
				message_index = i
				break
			end
			table.insert(donations, {player = params[i], score = params[i + 1]})
		end

		local dmessage = ""
		if message_index then
			dmessage = table.concat(params, " ", message_index)
			dmessage = (dmessage and dmessage ~= "") and (":" .. dmessage) or ""
		end

		if #donations == 0 then
			return false, "You should provide at least one player and score!"
		end

		local total_donation = 0
		for _, donation in ipairs(donations) do
			local pname = donation.player
			local score = tonumber(donation.score)
			...
			total_donation = total_donation + score
		end
		...
		if total_donation > cur_score / 2 then
			return false, "You can donate only half of your match score in total!"
		end
