...
local function update_playertag(player, t, nametag, team_nametag, symbol_nametag)
	...
	for n, extra in pairs(table.copy(nametag_players)) do
		...
		if setting == true then
			setting =
				ctf_settings.get(minetest.get_player_by_name(n), "teammate_nametag_style")
		end
...
ctf_modebase.features = function(rankings, recent_rankings)
	...
	local function send_death_message(player, killer, weapon_image)
		local death_setting =
			ctf_settings.get(minetest.get_player_by_name(player), "send_death_message")
			...
			for index, pname in ipairs(hitters) do
				...
				if index == 1 then
					assist_message = assist_message
						.. minetest.colorize(a_teamcolor, pname)
			...
			if player ~= killer then
				if weapon_message then
					local death_message = "You were "
						.. weapon_message
						.. " by "
						.. minetest.colorize(k_teamcolor, killer)
						.. assist_message
						.. "."
	...
	local function tp_player_near_flag(player)
		...
		if ctf_map.current_map.teams[tname] then
			pos = vector.offset(
				ctf_map.current_map.teams[tname].flag_pos,
				math.random(-1, 1),
				0.5,
				math.random(-1, 1)
			)
			...
			pos = vector.add(
				...
				vector.divide(ctf_map.current_map.size, 2)
			)
	...
	local function celebrate_team(teamname)
		for _, player in ipairs(minetest.get_connected_players()) do
			...
			local volume = (
				tonumber(ctf_settings.get(player, "flag_sound_volume")) or 10.0
			) / 10
		...
		if ctf_teams.get(player) then
			...
			if killer then
				...
				for _, pname in ipairs(healers) do
					recent_rankings.add(
						pname,
						{ score = math.ceil(killscore / #healers) }
					)
					total_enemy_reward = total_enemy_reward
						+ math.ceil(killscore / #healers)
				end

				recent_rankings.add(
					player,
					{ reward_given_to_enemy = total_enemy_reward },
					true
				)
	...
	return {
		...
		on_new_match = function()
			...
			local map_treasures =
				table.copy(ctf_modebase:get_current_mode().treasures or {})

			for k, v in
				pairs(
					ctf_map.treasure.treasure_from_string(ctf_map.current_map.treasures)
				)
			do
				map_treasures[k] = v
			end
			...
			if loading_screen_time then
				...
				minetest.after(
					math.max(0, LOADING_SCREEN_TARGET_TIME - total_time),
					function()
						mapload_huds:clear_all()
		...
		on_match_end = function()
			...
			if ctf_map.current_map then
				minetest.log(
					"action",
					"matchend: Match ended for map "
						.. ctf_map.current_map.name
						.. " in mode "
						.. (ctf_modebase.current_mode or "<nil>")
						.. ". Duration: "
		...
		team_allocator = function(player)
			...
			local pcount_diff_limit = (
				(players_diff <= math.min(one_fourth, 1))
				or (pkd >= 1.8 and players_diff <= math.min(one_third, 2))
			)
			if best_kd.kills + worst_kd.kills >= 30 then
				avg = actual_kd_diff
			end

			local result = pcount_diff_limit
				and (
					(best_kd.kills + worst_kd.kills >= 30 and best_kd.t == best_players.t)
					or (pkd >= math.min(1, kd_diff / 2) and avg >= 0.4)
				)
		...
		on_flag_take = function(player, teamname)
			...
			recent_rankings.add(pname, {
				score = ctf_teams.online_players[teamname].count
					* 3
		...
		on_flag_capture = function(player, teamnames)
			...
			for _, lost_team in ipairs(teamnames) do
				...
				local score = team_score
					/ math.max(1, ctf_teams.online_players[lost_team].count)
				score = math.max(
					8
						* (
							(player_scores[pname].flag_attempts or 0)
							+ math.min((os.time() - ctf_map.start_time) / 60, 10)
						),
			...
			local text =
				string.format(" has captured the flag and got %d points", capture_reward)
			if many_teams then
				text = string.format(
			...
			for teammate in pairs(ctf_teams.online_players[pteam].players) do
				if teammate ~= pname then
					...
					if table.indexof(healers, teammate) ~= -1 then
						teammate_value = teammate_value
							+ ((#ctf_teams.get_connected_players() / 10) / #healers)
					end
			...
			if teams_left <= 1 then
				...
				ctf_modebase.remove_bounties_on_teammates()
			end
		end,
		...
		get_chest_access = function(pname)
			...
			local pro_chest = player
				and player:get_meta():get_int(
						"ctf_rankings:pro_chest:" .. (ctf_modebase.current_mode or "")
					)
					>= 1
