	...
	return {
		add = function(player, amounts, no_hud)
			...
			for stat, amount in pairs(amounts) do
				rankings_players[player][stat] = (rankings_players[player][stat] or 0)
					+ amount

				if team then
					rankings_teams[team][stat] = (rankings_teams[team][stat] or 0)
						+ amount
					if stat == "score" then
						rankings_players[player][team .. "_" .. stat] = (
							rankings_players[player][team .. "_" .. stat] or 0
						) + amount
					end
				end
			end
