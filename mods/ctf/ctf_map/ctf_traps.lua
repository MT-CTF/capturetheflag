...
core.register_node("ctf_map:unwalkable_dirt", {
...
core.register_node("ctf_map:unwalkable_stone", {
...
core.register_node("ctf_map:unwalkable_cobble", {
...
core.register_node("ctf_map:spike", {
	description = S("Spike").."\n".."7 DPS",
	drawtype = "plantlike",
	...
	on_place = function(itemstack, placer, pointed_thing)
		...
		if pteam then
			if not ctf_core.pos_inside(pointed_thing.above, ctf_teams.get_team_territory(pteam)) then
				core.chat_send_player(placer:get_player_name(), S("You can only place spikes in your own territory!"))
				return itemstack
			end
...
for _, team in ipairs(ctf_teams.teamlist) do
	if not ctf_teams.team[team].not_playing then
		...
		core.register_node("ctf_map:spike_"..team, {
...
core.register_node("ctf_map:reinforced_cobble", {
	...
	on_punch = function(pos, node, digger)
		...
		if placer_team ~= digger_team then
			core.set_node(pos, {name = "ctf_map:reinforced_cobble_hardened"})
...
core.register_node("ctf_map:reinforced_cobble_hardened", {
	...
	on_punch = function(pos, node, digger)
		...
		local digger_team = ctf_teams.get(digger)
		if placer_team == digger_team then
			core.set_node(pos, {name = "ctf_map:reinforced_cobble"})
			meta = core.get_meta(pos)
			meta:set_string("placer_team", placer_team)
		end
	end,
